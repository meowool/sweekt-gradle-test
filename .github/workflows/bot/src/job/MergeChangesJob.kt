...
class MergeChangesJob(
  ...
  private suspend fun triggeredByDefaultBranch(
    ...
    val outputBranches = mutableListOf<GithubBranch>()
    ...
    // 3. Output merged branches
    return when (outputBranches.isEmpty()) {
      // If the output branches are empty, it means that we can terminate
      // the job chain directly
      true -> jobFailure()
      false -> jobSuccess(jobDataOf(Keys.Branches to outputBranches))
    }
  }
  ...
  private suspend fun onSuccessfulMerge(mergedBranch: Any) {
    ...
    git.checkout(mergedBranch).renameRemoteBranch(renamedBranch).push()
  }
}
