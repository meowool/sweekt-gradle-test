...
import com.meowool.sweekt.gradle.MavenMirrorRepository
...
val RepositoryHandler.mavenMirror: MavenMirrorRepository
    get() = MavenMirrorRepository()
 ...
 * @author chachako
 */
fun RepositoryHandler.maven(name: String, url: Any, snapshotsOnly: Boolean = false, action: MavenArtifactRepository.() -> Unit = {}) = maven {
    setName(name)
    setUrl(url)
 ...
 * @author chachako
 */
fun RepositoryHandler.maven(url: Any, snapshotsOnly: Boolean) = maven {
    setUrl(url)
    if (snapshotsOnly) mavenContent { snapshotsOnly() }
}
 ...
 * @author chachako
 */
fun RepositoryHandler.maven(url: Any, snapshotsOnly: Boolean, action: MavenArtifactRepository.() -> Unit) =
    maven {
        setUrl(url)
        if (snapshotsOnly) mavenContent { snapshotsOnly() }
        action()
